...
resource "kubernetes_config_map" "aws_auth_mgmt" {
  provider = kubernetes.mgmt
  count    = var.create && var.create_aws_auth_configmap && var.environment == "mgmt" ? 1 : 0
...
resource "kubernetes_config_map" "aws_auth_dev" {
  provider = kubernetes.dev
  count    = var.create && var.create_aws_auth_configmap && var.environment == "dev" ? 1 : 0
...
resource "kubernetes_config_map" "aws_auth_qa" {
  provider = kubernetes.qa
  count    = var.create && var.create_aws_auth_configmap && var.environment == "qa" ? 1 : 0
...
resource "kubernetes_config_map" "aws_auth_prod" {
  provider = kubernetes.prod
  count    = var.create && var.create_aws_auth_configmap && var.environment == "prod" ? 1 : 0
...
resource "kubernetes_config_map_v1_data" "aws_auth_mgmt" {
  count    = var.create && var.manage_aws_auth_configmap && var.environment == "mgmt" ? 1 : 0
  provider = kubernetes.mgmt
  ...
  depends_on = [
    ...
    kubernetes_config_map.aws_auth_mgmt,
  ]
}


resource "kubernetes_config_map_v1_data" "aws_auth_prod" {
  count    = var.create && var.manage_aws_auth_configmap && var.environment == "prod" ? 1 : 0
  provider = kubernetes.prod
...
resource "kubernetes_config_map_v1_data" "aws_auth_dev" {
  count    = var.create && var.manage_aws_auth_configmap && var.environment == "dev" ? 1 : 0
  provider = kubernetes.dev
  ...
  depends_on = [
    # Required for instances where the configmap does not exist yet to avoid race condition
    kubernetes_config_map.aws_auth_dev,
  ]
}

resource "kubernetes_config_map_v1_data" "aws_auth_qa" {
  count    = var.create && var.manage_aws_auth_configmap && var.environment == "qa" ? 1 : 0
  provider = kubernetes.qa
  ...
  depends_on = [
    # Required for instances where the configmap does not exist yet to avoid race condition
    kubernetes_config_map.aws_auth_qa,
  ]
}
